...
//	BYTE rx[2];
	...
	Status = XSpi_Transfer(&SpiInstance,tx,NULL,2);
	if(Status != XST_SUCCESS){
		xil_printf("SPI Write error!");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	Status = XSpi_Transfer(&SpiInstance,tx,rx,nbytes + 1);
	if(Status != XST_SUCCESS){
		xil_printf("SPI Write multiple-byte error!");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	return data + nbytes;
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	Status = XSpi_Transfer(&SpiInstance,tx,rx,2);
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	if(Status != XST_SUCCESS){
		xil_printf("SPI read error!");
		return 0xff;
	}
	return rx[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	if(Status != XST_SUCCESS){
		xil_printf("multiple-bytes register read error!");
		return data;
	}
	for(int i=0; i < nbytes; i++){
		data[i] = rx[i+1];
